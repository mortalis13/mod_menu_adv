 ...
 * @since       1.5
 */
abstract class ModMenuAdvHelper
{
	/**
	...
	public static function getModules($client=0){
	  $db     = JFactory::getDbo();
	  $query  = $db->getQuery(true)
	    ->select('a.name,a.element,a.extension_id')
	    ->from('#__extensions AS a')
	    ->where('a.type=\'module\' AND a.client_id='.$client);
	...
	public static function getStyles(){
	  $db     = JFactory::getDbo();
	  $query  = $db->getQuery(true)
	    ->select('a.id,a.template,a.title,a.client_id')
	...
	public static function getTemplates(){
	  $db     = JFactory::getDbo();
	  $query  = $db->getQuery(true)
	    ->select('a.name,a.element,a.extension_id,a.client_id')
	    ->from('#__extensions AS a')
	...
	public static function getPlugins1(){
	  $db     = JFactory::getDbo();
	  $query  = $db->getQuery(true)
	    ->select('a.name,a.extension_id,a.folder')
	    ->from('#__extensions AS a');
	...
	public static function getPlugins($type){
	  $db     = JFactory::getDbo();
	  $query  = $db->getQuery(true)
	    ->select('a.name,a.element,a.enabled,a.extension_id,a.folder,a.type')
	    ->from('#__extensions AS a')
	    ->where('a.type=\'plugin\' AND a.folder=\''.$type.'\'')
	    ->order('a.enabled desc');
